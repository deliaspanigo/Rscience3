...
# ------------------------------------------------------------------------------
# 3. OUTPUT FOLDER - Setup from YAML configuration
# ------------------------------------------------------------------------------
cat("📂 STEP 3: Setting up output folder...\n")

# Load configuration from _quarto.yml
tryCatch({
  ...
  if (is.null(output_folder)) {
    stop("Output folder not specified in _quarto.yml")
  }
  
  cat("   Output folder from config:", output_folder, "\n")
  ...
  # Clean existing content in output folder
  ...
  if (length(existing_items) > 0) {
    cat("   Cleaning", length(existing_items), "item(s) in output folder...\n")
    ...
    cat("   ✅ Output folder cleaned\n")
  } else {
    cat("   ✅ Output folder is already empty\n")
  }
  
}, error = function(e) {
  cat("   ❌ ERROR in output folder setup:", e$message, "\n")
  cat("   ⚠️  Continuing without output folder cleanup\n")
})
cat("\n")

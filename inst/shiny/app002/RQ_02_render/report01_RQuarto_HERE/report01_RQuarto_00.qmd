...
params:
    ...
    dataset_file_source: "from_params" 
    dataset_file_name: "from_params"
---
...
# Output file names
str_output_file_name_Env_RData <- config$list_output_file_names$str_file_name_Env_RData
```
...
if(activate_params){
    ...
    dataset_file_source <- params$"dataset_file_source"
    if(dataset_file_source == "from_params") dataset_file_source <- "r_source"
    dataset_file_name <- params$"dataset_file_name"
    if(dataset_file_name == "from_params") dataset_file_name <- "mtcars"
    var_name_rv <- "mpg"
    var_name_factor <- "cyl"
    alpha_value <- 0.05
    ...
    check_r_source        <- dataset_file_source == "r_source"
    check_rscience_source <- dataset_file_source == "rscience_source"
    check_excel_source    <- dataset_file_source == "excel_source"
    check_csv_source      <- dataset_file_source == "csv_source"

    if(check_r_source) my_dataset <- get(dataset_file_name)
...
{{< include report01_RQuarto_file01_sec13_Descriptive_RV.qmd >}}

# Section 14 - Descritive for Residuals
{{< include report01_RQuarto_file02_sec14_Descriptive_Residuals.qmd >}}

<!-- 15 to 17 -->
{{< include report01_RQuarto_file03_sec15_to_17_ASA.qmd >}}
...
save(list = vector_save_obj, file = str_output_file_name_Env_RData)

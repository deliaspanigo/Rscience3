#' Executes Step 06: Renders the Docx Quarto document (report04_docx.qmd).
#'
...
fn_proc_step06_quarto_report04 <- function(state, my_pipeline_steps, list_bag_steps, set_default_quiet, advance_step_trigger) {
  
  # ------------------------------------------------------------------
  # Step 06: Rendering the Docx Quarto document.
  # ------------------------------------------------------------------
  observeEvent(state$current_step, {
    
    # 1. Requirements
    # [AJUSTE CLAVE] El Step 06 debe requerir state$current_step == 6
    req(state$current_step == 6)
    req(!my_pipeline_steps$steps$step06$check)
    print("Step 06 - Init (Async)")
    
    # 2. State info - Update UI immediately
    state$message <- my_pipeline_steps$steps$step06$info_step
    ...
    str_subfolder <- "report04_docx_HERE"
    str_input_file_name <- "report04_docx.qmd"
    ...
    future_promise({
      ...
      pattern_to_check <- "^zzz_output_report04"
      ...
      # 5. Step conclusion
      if (result$success) {
        # [AJUSTE] Usar un nombre clave para el Docx (Reporte 04)
        list_bag_steps$final_report_04_path <- result$output_path
        
        my_pipeline_steps$steps$step06$check <- TRUE
        my_pipeline_steps$steps$step06$status <- "Done! (Report 04 Docx Generated)"
      } else {
        # Manejo de advertencia
        my_pipeline_steps$steps$step06$check <- TRUE
        my_pipeline_steps$steps$step06$status <- "Warning! (Output file missing)"
        state$message <- "WARNING: Quarto ran, but the Docx output file was not found."
      }
      
      # [AVANCE DESACOPLADO] Avanzar al Step 07
      current_step_value <- state$current_step
      advance_step_trigger(current_step_value + 1)
      print("Step 06 - End (Async)")
      
    } %...!% {
      # --- MANEJO DE ERRORES (CRASH) ---
      error_message <- paste("Fatal Error in Step 06 (Quarto Docx Rendering):", .$message)
      warning(error_message)
      state$message <- error_message
      my_pipeline_steps$steps$step06$status <- "Quarto Crash!"
    }

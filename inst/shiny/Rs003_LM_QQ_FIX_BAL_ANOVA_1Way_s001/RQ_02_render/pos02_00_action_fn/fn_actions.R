...
fn_copy_verify_single_file <- function(str_from, str_to) {
  ...
  base::cat(base::paste0("<small style='color: #6c757d;'><b>FROM:</b>&nbsp;<code>", str_from, "</code></small><br>"))
  base::cat(base::paste0("<small style='color: #6c757d;'><b>TO:</b>&nbsp;&nbsp;&nbsp;<code>", str_to, "</code></small><br>"))
  ...
  status_from <- base::ifelse(fs::file_exists(str_from),
...
fn_check_and_create_and_clean_single_folder_SUPER_POWER <- function(str_folder_path) {
  ...
  # 1. Security String Configuration
  str_security_key <- "zzz_zzz_output"
  ...
  # 3. SUPER POWER SECURITY CHECK: Keyword Whitelist
  if (!base::grepl(str_security_key, str_folder_path_tidy, fixed = TRUE)) {
    base::cat(base::paste0("<span style='color: #dc3545;'>✘ <b>Super Power Security Alert:</b> Unauthorized path.</span><br>"))
    base::cat(base::paste0("<small style='color: #6c757d;'>The path must contain the security key: <code>", str_security_key, "</code></small><br><br>"))
    return(FALSE)
  }

  # HTML Header for Quarto Output
  base::cat(base::paste0("<b>Managing folder lifecycle (SUPER POWER):</b> <code>", str_folder_path_tidy, "</code><br>"))
  base::cat(base::paste0("<small style='color: #6c757d; font-style: italic;'>Action: Authorized external/internal lifecycle management.</small><br>"))

  # 4. Execute logic within tryCatch
  result <- base::tryCatch({

    if (fs::dir_exists(str_folder_path)) {
      # --- CASE A: EXISTS -> EMPTY CONTENT ---
      base::cat("<span style='color: #6c757d;'>ℹ <b>Info:</b> Authorized directory exists. Cleaning content...</span><br>")

      content <- fs::dir_ls(str_folder_path, all = TRUE)

      if (base::length(content) > 0) {
        fs::file_delete(content)
        base::cat("<span style='color: #28a745;'>✔ <b>Success:</b> Content purged successfully.</span><br>")
      } else {
        base::cat("<span style='color: #6c757d;'>ℹ <b>Info:</b> Directory was already clean.</span><br>")
      }

    } else {
      # --- CASE B: MISSING -> CREATE ---
      base::cat("<span style='color: #6c757d;'>ℹ <b>Info:</b> Authorized directory missing. Creating...</span><br>")
      fs::dir_create(str_folder_path, recurse = TRUE)
      base::cat("<span style='color: #28a745;'>✔ <b>Success:</b> Directory structure established.</span><br>")
    }

    # --- FINAL VERIFICATION & METADATA AUDIT ---
    if (fs::dir_exists(str_folder_path)) {
      # Extracting metadata for the authorized path
      folder_info <- fs::file_info(str_folder_path)
      folder_perms <- folder_info$permissions
      file_count <- base::length(fs::dir_ls(str_folder_path, all = TRUE))

      base::cat("<span style='color: #28a745;'>✔ <b>Verification:</b> Path is active and clean.</span><br>")
      base::cat(base::paste0("<small style='color: #6c757d;'><b>Super Power Audit:</b> Permissions: <code>", folder_perms,
                             "</code> | Final Item Count: <b>", file_count, "</b></small><br><br>"))
      TRUE
    } else {
      base::stop("Post-condition failed: Directory not found after operation.")
    }

  }, error = function(e) {
    base::cat("<span style='color: #dc3545;'>✘ <b>Error:</b> Operation failed. Check network/disk permissions.</span><br>")
    base::cat(base::paste0("<small style='color: #b02a37;'>Technical Detail: ", e$message, "</small><br><br>"))
    FALSE
  })
...
fn_copy_verify_multi_files <- function(vector_from, vector_to) {
  ...
  vector_from <- fs::path_tidy(vector_from)
  vector_to   <- fs::path_tidy(vector_to)
...
fn_check_multi_files_pattern <- function(vector_paths, str_regex) {
  library("fs")
  library("dplyr")
  library("knitr")
  library("kableExtra")
  ...
  if (base::length(vector_paths) == 0) return(TRUE)
  ...
  df_audit <- base::data.frame(
    Full_Path = vector_paths,
    File_Name = fs::path_file(vector_paths),
    stringsAsFactors = FALSE
  ) %>%
    dplyr::mutate(
      `#` = dplyr::row_number(),
...
fn_export_files_zzz_output <- function(str_path_from, str_path_to,
  ...
  # 2. Report Header
  base::cat(base::paste0("<b>Export Audit:</b> <code>zzz_output*</code><br>"))
  ...
  # 3. Destination Folder Display (NEW)
  base::cat(base::paste0(
  ...
  # 4. File Listing and Analysis
  # Filtering files that contain 'zzz_output' in their path
  list_files <- fs::dir_ls(str_path_from, recurse = TRUE, regexp = "zzz_output")

  if (base::length(list_files) == 0) {
    base::cat("<span style='color: #6c757d;'>ℹ <b>Info:</b> No files found containing 'zzz_output'.</span><br><br>")
    return(TRUE)
  }
  ...
  audit_data <- base::lapply(base::seq_along(list_files), function(i) {
    current_full_path <- list_files[i]
    ...
    base::data.frame(
      ...
      Size = base::format(f_info$size, units = "auto"),
      Status = status_label,
      Action = action_label,
      execute = do_copy,
  ...
  base::cat(base::paste0(
    ...
    "<small style='color: #333;'><b>Scan:</b> Detected <b>", base::length(list_files),
    "</b> files.</small></div>"
  ))

  # 8. Generate Table with Relative Paths
  html_table <- df_audit %>%
    dplyr::mutate(
    ...
    dplyr::select(`#`, Relative_Path, Size, Status, Action) %>%
    knitr::kable(format = "html", escape = FALSE, align = "cllll",
                 col.names = c("#", "Relative Export Path", "Size", "Status", "Action")) %>%
    kableExtra::kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                              full_width = F, position = "left") %>%
    kableExtra::column_spec(1, color = "#6c757d", width = "40px") %>%
  ...
  if (base::isTRUE(check_export)) {
    base::cat("<span style='color: #28a745;'>✔ <b>Audit Complete:</b> All files processed successfully.</span><br><br>")
    return(TRUE)
  } else {
...
fn_render_qmd_files <- function(vector_files) {
  library("quarto")
  library("dplyr")
  library("knitr")
  library("kableExtra")

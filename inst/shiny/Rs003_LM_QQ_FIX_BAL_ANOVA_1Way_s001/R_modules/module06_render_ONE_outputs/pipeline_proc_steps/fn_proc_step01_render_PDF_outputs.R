fn_proc_step01_render_PDF_outputs <- function(state, my_pipeline_steps, list_bag_steps, session, advance_step_trigger) {
    ...
    shinyjs::delay(1000, {
      ...
      future_promise({
        ...
        if (result$success) {
          ...
          my_pipeline_steps$steps$step01$status <- "Done! (Report Generated)"
          print("TODO OK!")

        } else {
          # [WARNING] Quarto ran, but verification failed.
          my_pipeline_steps$steps$step01$check <- TRUE # Mark as completed to advance
          my_pipeline_steps$steps$step01$status <- "Warning! (Output files missing)"
        ...
        error_message <- paste("Fatal Error in Step 01 (Quarto Rendering):", .$message)
        warning(error_message)
        state$message <- error_message
        my_pipeline_steps$steps$step01$status <- "Quarto Crash!"

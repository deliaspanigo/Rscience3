#' Theory Module UI
module_work_table_ui <- function(id) {
  ns <- NS(id)
...
module_work_table_server <- function(id, app_state) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    my_pipeline_steps <- reactiveValues(
      steps = list(
        ...
        "step02" = list(
          id = "step02",
          info_step = "Taking R scripts from Rscience bank code...",
          check = FALSE,
          status = "waiting...",
          func = function() {
        ...
        "step03" = list(
          id = "step03",
          info_step = "Preparing a new R script using your input information...",
          check = FALSE,
          status = "waiting...",
          func = function() {
        ...
        "step04" = list(
          id = "step04",
          info_step = "Running your new R script...",
          check = FALSE,
          status = "waiting...",
          func = function() {
    ...
    observe({
      state$max_step <- length(reactiveValuesToList(my_pipeline_steps)$"steps")
    })
    # ==========================================================================
    ...
    if(check_modal){
      ...
      ")
            ...
            fluidRow(
              ...
              column(6,
                     div(style = "text-align: center;",
                         uiOutput(ns("modal_status_icon")) # <--- Cambiado de icon(...) a uiOutput
                     )
              )
            ),
          ...
          # footer = NULL,
          footer = uiOutput(ns("modal_footer_button")),
          easyClose = FALSE,
          size = "xl"
        )
      }
    ...
    output$modal_status_icon <- renderUI({
      # Si el paso actual es mayor al máximo, mostramos el Check Verde
      if (state$current_step > state$max_step) {
        icon("check-circle", class = "fa-3x", style = "color: #198754;") # Verde Success
      } else {
        # Mientras esté procesando, mostramos el Spinner
        icon("spinner", class = "fa-spin fa-3x", style = "color: #0d6efd;") # Azul Primary
      }
    })
    ...
    output$modal_footer_button <- renderUI({
      ns <- session$ns
      if (state$current_step > state$max_step) {
        actionButton(
          ns("close_modal_btn"),
          "Finish & Close",
          class = "btn-success btn-lg", # Botón verde grande
          style = "font-weight: bold; width: 100%;"
        )
      } else {
        # Opcional: un botón deshabilitado o nada
        NULL
      }
    })
    ...
    observeEvent(input$close_modal_btn, {
      removeModal()
      the_closing(TRUE)
      print("User closed the modal manually.")
    })
    ...
    str_steps_folder_path <- file.path("R_modules", "module04_work_table", "pipeline_proc_steps")
    ...
    fn_proc_step03_modifying_files(state, my_pipeline_steps, list_bag_steps, session, advance_step_trigger)
    fn_proc_step04_run_Rscript_internal(state, my_pipeline_steps, list_bag_steps, session, advance_step_trigger)
